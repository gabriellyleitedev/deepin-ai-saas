...
export default function AiPlasmaGlobe({ isResponding = false }) {
  return (
    <div className="relative w-80 h-80 flex items-center justify-center select-none overflow-visible">
      
      {/* 1. AURA VOLUMÉTRICA TRASEIRA (Iluminação de estúdio baseada nas cores da imagem) */}
      <div 
        className={`absolute w-72 h-72 rounded-full blur-[60px] pointer-events-none transition-all cubic-bezier(0.16, 1, 0.3, 1) duration-1000 ${
          isResponding 
            ? "scale-110 bg-gradient-to-tr from-cyan-500/40 via-fuchsia-500/30 to-amber-500/30 opacity-100" 
            : "scale-95 bg-gradient-to-tr from-purple-600/20 via-indigo-500/10 to-transparent opacity-40 animate-pulse"
        }`} 
        style={{ animationDuration: '3.5s' }}
      />

      {/* 2. CONTENÇÃO DO GLOBO EM SVG DE ALTA FIDELIDADE */}
      <svg 
        viewBox="0 0 200 200" 
        className={`w-full h-full drop-shadow-[0_30px_60px_rgba(0,0,0,0.95)] transition-transform cubic-bezier(0.16, 1, 0.3, 1) duration-700 ${
          isResponding ? "scale-[1.03]" : ""
        }`}
      >
        <defs>
          {/* Máscara cirúrgica para garantir o formato esférico perfeito */}
          <clipPath id="plasmaBoundary">
            <circle cx="100" cy="100" r="75" />
          </clipPath>

          {/* Ele deforma a imagem sutilmente no modo ocioso e acelera quando a IA responde */}
          ...
          {/* Sombra de contorno interno (Efeito Fresnel de lente esférica espessa) */}
          ...
          {/* Contra-luz inferior ciano (Rim light) baseada no reflexo azul da imagem */}
        ...
        {/* CAMADA 1: Base de fundo escuro absoluto */}
        <circle cx="100" cy="100" r="75" fill="#030206" />

        {/* --- CONTEÚDO DA IMAGEM VIVA (ENCAPSULADO NO VIDRO) --- */}
        <g clipPath="url(#plasmaBoundary)">
          
          {/* CAMADA 2: A sua Imagem com o filtro líquido aplicado */}
          <image 
            href={aiCoreImage} 
            x="20" 
            y="20" 
        ...
        {/* CAMADA 4: ACABAMENTO DA SUPERFÍCIE ÓPTICA (Vidro Fotorrealista) */}
        ...
        {/* Contra-reflexo sutil de estúdio no canto oposto */}
