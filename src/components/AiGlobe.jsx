...
export default function AiGlobe({ isResponding = false }) {
  return (
    <div className="relative w-72 h-72 flex items-center justify-center select-none">
      ...
      {/* 1. GLOW VOLUMÉTRICO TRASEIRO */}
      <div
        className={`absolute w-64 h-64 blur-[65px] rounded-full pointer-events-none transition-all cubic-bezier(0.16, 1, 0.3, 1) duration-1000 ${isResponding
      ...
      <svg
        viewBox="0 0 200 200"
        className={`w-full h-full drop-shadow-[0_25px_50px_rgba(0,0,0,0.9)] transition-transform cubic-bezier(0.16, 1, 0.3, 1) duration-700 ${isResponding ? "scale-[1.03]" : ""
          }`}
      >
        <defs>
          ...
          <radialGradient id="deepinCoreGlow" cx="38%" cy="34%" r="68%" fx="30%" fy="25%">
            ...
            <stop offset="55%" stopColor="#3730a3" stopOpacity="0.9" />  {/* Ponto frio interno */}
          ...
          {/* REFLEXOS DE CURVATURA DA LENTE  */}
          ...
          <linearGradient id="rimLight" x1="0%" y1="100%" x2="0%" y2="0%">
            <stop offset="0%" stopColor="#22d3ee" stopOpacity="0.4" />   {/* Brilho ciano inferior */}
            <stop offset="45%" stopColor="#a855f7" stopOpacity="0.05" />
            <stop offset="100%" stopColor="#ffffff" stopOpacity="0" />
          </linearGradient>

          {/* FILTRO DE ENERGIA ORGÂNICA (Simula refração fluida ) */}
        ...
        <g clipPath="url(#globeBoundary)">
          ...
          {/* Vínculo de Sombra (Efeito Fresnel interno para dar peso e volume) */}
          <circle cx="100" cy="100" r="72" fill="none" stroke="#020408" strokeWidth="10" opacity="0.5" />
        </g>

        {/* FIM DO CONTEÚDO ENCAPSULADO */}

        {/* CAMADA 4: TRATAMENTO DE SUPERFÍCIE (O efeito "Bola de Cristal") */}
        {/* Sombra de contorno externa */}
        ...
        {/* Micro-borda de luz branca superior */}
        <circle cx="100" cy="100" r="71.5" fill="none" stroke="#ffffff" strokeWidth="0.5" opacity="0.3" />

        {/* CAMADA 5: (Simula a curvatura externa capturando luz ambiente) */}
        <path
        ...
        {/* CAMADA 6: (Simula a curvatura interna capturando luz ambiente) */}
        {/* Brilho secundário suave de fundo */}
